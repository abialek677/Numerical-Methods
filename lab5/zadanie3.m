...
for i = 1:length(N)
    V_matrix = vandermonde_matrix2(N(i));
    matrix_condition_numbers(i) = cond(V_matrix);
end
...
for i = 1:length(N)
    ...
    title('Growth of condition numbers')

    subplot(3,1,2)
    plot(N, max_coefficients_difference_1)
    xlabel('Matrix size')
    ylabel('Max coefficient difference')
    title('Growth of max coefficient difference - linear b values')

    subplot(3,1,3)
    plot(N, max_coefficients_difference_2)
    xlabel('Matrix size')
    ylabel('Max coefficient difference')
    title('Growth of max coefficient difference - disturbed linear b values')
